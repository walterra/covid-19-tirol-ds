...
const parse = (id) => {
  ...
  const name = $('.single-death-header h3').text().trim();
  const hash = hashCode(name);
  const date = $('.single-death-header .die_date').text().replace('†', '').trim();
  const ls = $('.death-deatails-left-col span').last();
  const l = $(ls).text().replace('Ort:', '').trim();
  ...
  if (location !== 'SKIP' && location !== '' && location.includes(',') === false) {
    ...
    if (district === undefined && unknownLocations[location] === undefined) {
      ...
      return [[date,location,district,hash].join(',')];
    }
  }
