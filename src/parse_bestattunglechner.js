...
const parse = (id) => {
  ...
  const deceasedItem = $('.sterbefall');

  const data = [];

  deceasedItem.each((i, e) => {
    const name = $(e).find('.details h4').text().replace(/([A-Z])/g, ' $1').trim();
    const hash = hashCode(name);
    const date = $(e).find('.sterbetag').text().replace('†', '').trim();
    const l = $(e).find('.sterbefall-overlay span').last().text().trim();

    const location = orte[l] !== undefined ? orte[l] : l;
    if (date !== '' && location !== 'SKIP' && location !== '' && location.includes(',') === false) {
      const district = gemeinden[location];
      if (district === undefined && unknownLocations[location] === undefined) {
        console.error(`Unknown location in file ${id}: ${date}, ${name}, ${location}`);
        // console.error(`${[location].join(',')}`);
        //console.error(`----`);
        unknownLocations[location] = location;
        // process.exit();
      } else {
