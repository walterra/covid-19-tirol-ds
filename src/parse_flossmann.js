...
const parse = (id) => {
  ...
  const deceasedItem = $('.sterbefall-card');

  const data = [];

  deceasedItem.each((i, e) => {
    const name = $(e).find('.line.one').text().trim();
    const hash = hashCode(name);
    const date = $(e).find('.line.three').text().trim().replace('†', '');
    const l = $(e).find('.line.four').text().trim();
