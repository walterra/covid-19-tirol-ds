...
const parse = (id) => {
  ...
  const deceasedItem = $('.sf-box');

  const data = [];

  deceasedItem.each((i, e) => {
    const name = $(e).find('.sf-name').text().trim();
    const hash = hashCode(name);
    const date = $(e).find('.sf-sterbetag').text().replace('†', '').trim();
    const l = $(e).find('.sf-bestattungsort').text().trim();
