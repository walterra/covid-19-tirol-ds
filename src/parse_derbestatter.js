...
const parse = (id) => {
  ...
  const deceasedItem = $('.death_card_left_side');

  const data = [];

  deceasedItem.each((i, e) => {
    const nameArr = $(e).find('h3').text().trim().split(' ')
    nameArr.pop();
    const name = nameArr.join(' ');
    const hash = hashCode(name);
    const date = $(e).find('.die_date').text().replace('†', '').trim();
    const l = $(e).find('.death_buttons').find('div').first().text().trim().replace('Ort: ', '');

    // console.log(`${name} —— ${date} —— ${l}`);
